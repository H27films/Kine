...
const TrackerSparkline: React.FC<Props> = ({ weekChartData }) => {
  ...
  const yMax = Math.max(dataMax, Y_MIN + 1);
  ...
  const getY = (val: number) => {
    const clamped = Math.max(Y_MIN, Math.min(val, yMax));
    return padTop + chartH - ((clamped - Y_MIN) / range) * chartH;
  };

  const lineVals: number[] = weekChartData.map((val) => {
    if (val > 0) return val;
    return Y_MIN;
  });
  ...
  const linePts = lineVals.map((val, i) => ({
    ...
    val,
    i,
    isAnchor: weekChartData[i] <= 0
  }));

  // Build path connecting all points
  let pathD = `M ${linePts[0].x} ${linePts[0].y}`;
    ...
    <svg width="100%" viewBox={`0 0 ${VW} ${VH + 14}`} style={{ overflow: 'visible', display: 'block' }}>
      ...
      {linePts.filter(p => !p.isAnchor).map((p, k) => (
        <g key={k}>
          ...
          <text x={p.x} y={p.y - 6} textAnchor="middle" fill="rgba(255,255,255,0.70)" fontSize="6" fontWeight="700">
            {p.val.toFixed(1)}
          </text>
        </g>
      ))}
