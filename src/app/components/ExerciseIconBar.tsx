...
const IconSVG: React.FC<{ iconKey: string; color: string }> = ({ iconKey, color: c }) => {
  switch (iconKey) {
    case 'running':
      return (
    ...
    case 'walking':
      return (
        <svg viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
          ...
          <circle cx="25" cy="40" r="3" fill={c}/><circle cx="32" cy="35" r="3" fill={c}/>
          <circle cx="40" cy="35" r="3" fill={c}/><circle cx="48" cy="40" r="3" fill={c}/>
          <path d="M60 70C60 80 70 85 75 80C80 75 75 60 70 50C65 40 55 40 55 50C55 60 60 60 60 70Z" fill={c}/>
          <circle cx="75" cy="40" r="3" fill={c}/><circle cx="68" cy="35" r="3" fill={c}/>
          <circle cx="60" cy="35" r="3" fill={c}/><circle cx="52" cy="40" r="3" fill={c}/>
        </svg>
      );
    case 'cycling':
      return (
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      {ICON_KEYS.map(key => {
        const ex = exercises.find(e =>
          e.exercise_name?.toLowerCase().replace(/[-\s]/g, '') === key
        );
        if (!ex) return null;
