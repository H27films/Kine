    ...
    <div>
      ...
      <section className="mb-12">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div onClick={() => setSelectedMuscle('chest')} className="p-6 rounded-lg flex flex-col justify-between aspect-square cursor-pointer active:scale-95 duration-200" style={{ backgroundColor: '#2a2a2a', border: selectedMuscle === 'chest' ? '1px solid rgba(255,255,255,0.2)' : '1px solid rgba(255,255,255,0.05)' }}>
            <Dumbbell size={32} color="white" />
            <span className="font-black uppercase tracking-tight text-xl text-white">Chest</span>
          </div>
          <div className="grid grid-rows-2 gap-3">
            {muscleGroups.map((mg) => (
              <div key={mg.id} onClick={() => setSelectedMuscle(mg.id)} className="p-4 rounded-lg flex items-center justify-between cursor-pointer active:scale-95 duration-200" style={{ backgroundColor: '#1b1b1b', border: selectedMuscle === mg.id ? '1px solid rgba(255,255,255,0.2)' : '1px solid transparent' }}>
          ...
          <div onClick={() => setSelectedMuscle('shoulders')} className="col-span-2 p-4 rounded-lg flex items-center justify-between cursor-pointer active:scale-95 duration-200" style={{ backgroundColor: '#1b1b1b', border: selectedMuscle === 'shoulders' ? '1px solid rgba(255,255,255,0.2)' : '1px solid transparent' }}>
      ...
      <section className="mb-12 p-8 rounded-xl" style={{ backgroundColor: '#0e0e0e', border: '1px solid rgba(71,71,71,0.1)', boxShadow: '0 25px 50px rgba(0,0,0,0.5)' }}>
        <div className="mb-8">
          ...
          <div className="flex items-baseline gap-2">
            <input type="number" value={weight} onChange={(e) => setWeight(e.target.value)} placeholder="00" className="text-6xl font-black tracking-tighter text-white w-32 p-0" style={{ backgroundColor: 'transparent', border: 'none', color: '#ffffff' }} />
